...
exports.saveActivities = function (data, start_time, watch_token, next) { 
  ...
  Activity.count({time: {$lt: end, $gt: start}, watch: watch_token}, function (err, count) {
    if (err) return next(err)
    if (count > 0) return next(new Error('Already have data for ' + start_time))
...
exports.get_n_days_before_now = function (n, watch_token, next) {
  ...
  .exec(function (err, latest_activity) {
    if (err) return next(err)
    var latest_time = new Date(latest_activity.time)
